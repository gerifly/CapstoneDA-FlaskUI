...
def index():
    ...
    stats = {
        ...
        'rev_table' : playstore.groupby(['Category','App']).agg({'Reviews' : 'sum','Rating' : 'mean'}).sort_values(by = 'Reviews', ascending = False).head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    plt.savefig('order_type.png',bbox_inches="tight")
    ...
    plt.title('Density Plot with Size')
    plt.xlabel('Size')
    plt.ylabel('Density')
    ...
    return render_template('index.html', stats=stats, result=result, result2=result2, result3=result3,result4=result4,result5=result5)
